...
#seeds from 0 to 5to make results reproducible, and select the same starting display for each method
seeds = list(range(5))
...
max_iter_list = [3] #we select teh paparmeter that gives the best result at iteration 3 
...
k_pos_neg_list = [(3,3),(3,0),(-1,-1),(-1,0)]
...
output_filename=f"out_results/clip/grid_search/exps_results_grid_search_poly_msed.jsonl"
                    ...
                    for dataset in datasets:
                        ...
                        for max_iter in max_iter_list:   
                            ...
                            for use_batch in [True, False]:
                                method="Polyquery_msed_logscale"
                                for data_preprocessing in ["softmax"]:
                                    new_exp_params=get_params(exp_params,method,"msed", data_preprocessing, use_batch)
                                    alpha_values=[0.25, 0.5, 0.75, 1]
                                    beta_values=[0.25, 0.5, 0.75, 1]
                                    ...
                                    for alpha in alpha_values:
                                        for beta in beta_values:
                                            for gamma in gamma_values:
                                                if alpha+beta-gamma!=1 or (beta+gamma==0):
                                                    continue
